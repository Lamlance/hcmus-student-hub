...
var globalRtcMap GlobalRtcKeyStore = GlobalRtcKeyStore{
	mu: &sync.Mutex{},
...
func (s *Service) GenRtcToken(tokenRequest TokenRequest) (string, error) {
	...
	globalRtcMap.mu.Lock()
	defer globalRtcMap.mu.Unlock()

	currSec := time.Now().Unix()

	val, ok := globalRtcMap.v[tokenRequest.Channel]
	if !ok || val.uid != uint32(uid64) || val.exp <= currSec {
		token, _err := rtctokenbuilder2.BuildTokenWithUid(
			s.appID, s.appCertificate, tokenRequest.Channel,
		...
		globalRtcMap.v[tokenRequest.Channel] = struct {
			exp   int64
			uid   uint32
			token string
		}{
			exp:   currSec + int64(tokenRequest.ExpirationSeconds),
			uid:   uint32(uid64),
			token: token,
		}
		log.Println("TOKEN: " + strconv.FormatUint(uint64(uid64), 10) + token)
		return token, _err
	}
	log.Println("TOKEN: " + strconv.FormatUint(uint64(uid64), 10) + val.token)
